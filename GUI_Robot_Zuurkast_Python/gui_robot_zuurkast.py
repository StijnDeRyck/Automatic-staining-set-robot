...
class GUI_Robot_Zuurkast(QMainWindow, Ui_MainWindow):
    def __init__(self, port=None):
        ...
        self.GrabBasket_Zdist = self.widget_visu.type.LiquidSt[4] - self.widget_visu.type.LiqBoxIn_height + \
                                self.widget_visu.type.Basket_height + self.widget_visu.type.BasketHandle_height - self.originZgripper
        self.MoveBasket_Zdist = self.GrabBasket_Zdist + self.widget_visu.type.Basket_height + self.widget_visu.type.BasketHandle_height - 10  # + 20
    ...
    def check_finished(self):
        ...
        #device = self.device
        time.sleep(0.1)
        response = wait_for_response(self.device)
        # print("Ready?",end="")
    ...
    def home(self):
        ...
        print("homing")
        print(self.GrabBasket_Zdist +10)
        self.device.write(bytearray("Home " + str(self.GrabBasket_Zdist + 10) + " " + str(self.GripperClamp) + " " + str(self.GripperOpen) + "\r", "utf8"))
        if self.widget_visu.NewSeq_toggling or self.widget_visu.EditSeq_toggling:  # if we are making or editing a sequence, add the command to the sequence
            self.widget_visu.type.sequence_list[self.SeqNum].append(bytearray("Home" + str(self.GrabBasket_Zdist + 10) + " " + str(self.GripperClamp) + " " + str(self.GripperOpen) + "\r", "utf8"))
    ...
    def MoveBasket(self):
        try:
            ...
            command_list = []
            command_list.extend([bytearray("M_abs 2 " + str(self.MoveLid_Zdist) + "\r", "utf8")] +
                                 self.widget_visu.type.move_to_fixed_pos(self.ID_current) +
