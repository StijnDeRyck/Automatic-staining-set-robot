...
void check_move_complete() {
  ...
  for (int i = 0; i <= (NumOfNemaSteppers-1); i++) {
    if (stepperNema[i].distanceToGo() != 0) {
      ...
      Serial.print("Nema stepper ");
      Serial.print(i);
      Serial.print(" still runing;  distance to go = ");
      Serial.println(stepperNema[i].distanceToGo());
  ...
  for (int i = 0; i <= (NumOfCheapSteppers-1); i++) {
    if (stepperCheap[i].getStepsLeft() != 0) {
      ...
      Serial.print("Cheap stepper ");
      Serial.print(i);
      Serial.print(" still runing;  steps left = ");
      Serial.println(stepperCheap[i].getStepsLeft());
...
void homing(){
  char *arg;
  int safeZdist = 70; //[mm]  default value
  int clamp_distance = 60; //[mm]  default value
  int GripperOpen = -25; //[mm]  default value
  stepperNema[0].setMaxSpeed(800);
  stepperNema[1].setMaxSpeed(800);
  stepperNema[2].setMaxSpeed(800);
  
  stepperCheap[0].newMoveDegrees(false, 8*gearRatioCheap[0]);  // release object
  
  arg = SCmd.next();
  if (atoi(arg) != NULL) {
    if (atoi(arg) < 0) {
      Serial.print("Not recognized safe Z distance: ");
      Serial.println(safeZdist);
      return;
    }
    safeZdist = atoi(arg);
  }
  ...
  arg = SCmd.next();
  if (atoi(arg) != NULL) {
    clamp_distance = atoi(arg);
  }
  arg = SCmd.next();
  if (atoi(arg) != NULL) {
    GripperOpen = atoi(arg);
  }
  
  stepperNema[2].move(safeZdist*17.77778*gearRatioNema[2]);
  stepperNema[2].run();
  b_move_complete = false;
  while (b_move_complete == false) {
    stepperCheap[0].run();
    stepperNema[2].run();
    
    if (millis() - Home_lastMillis >= 500UL) {
      Home_lastMillis = millis();
      check_move_complete();
    }
  }
  stepperNema[0].move(-100000);
  stepperCheap[0].newMoveDegrees(true, clamp_distance*gearRatioCheap[0]);
  while (xHomed == false) {
    stepperNema[0].run();
    stepperCheap[0].run();
    limitswitch();
  ...
  stepperCheap[0].newMoveDegrees(false, abs(GripperOpen)*gearRatioCheap[0]);
